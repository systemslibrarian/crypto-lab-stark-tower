 ...
 *    an unknown backdrop, or a `color-mix()`. This page is built out of all
 *    three. `<body>` paints two `radial-gradient(… at 5% -10%)` /
 *    `(… at 95% -5%)` washes over `--bg`; every panel surface is one of five
 *    translucent `--overlay-*` tokens (`rgba(19,28,46,0.94)` down to
 *    `rgba(13,20,34,0.65)`); and every verdict, chip, badge, callout and
 *    trust-banner is a `color-mix(in srgb, var(--ok|--err|--warn) N%,
 *    transparent)` on top of that. axe files those under `incomplete`, which
 *    means a violations-only gate measured the contrast of almost nothing this
 *    lab renders — and two of them were real failures: `.badge-real` at 2.90:1
 *    and `.callout strong` at 3.74:1, both in the light theme.
 ...
 * along it, which matters here: the eight `.badge-real` chips run down a page
 * several screens tall and the body wash is strongest at the top, so the SAME
 * badge measures 2.90:1 in Exhibit 01 and 3.33:1 in Exhibit 05.
 *
 ...
 *    boxes are the four `.table-wrap`s (`overflow-x: auto` around a
 *    `.commit-table` that forces `min-width: 660px`), the five `pre.log`
 *    regions (`max-height: 220px; overflow: auto`, which the prover transcript
 *    and the query inspector both exceed), and every `.panel`, which is
 *    `overflow: hidden`. Content scrolled past those is not dimmed or partly
 ...
 *    failure. Skip it, and rely on the content that IS in view.
 *
 ...
 *    load-bearing here rather than defensive: `renderFriViz()` builds an inline
 *    `<svg>` whose `<text class="viz-label">` row labels sit beside `<line>`
 *    and `<circle>` siblings. An ancestor-only walk would composite those
 *    labels onto the `<svg>`'s own transparent background instead of onto what
 *    is drawn under them, and report a ratio nothing on screen has. SVG text
 *    also takes its ink from `fill`, not `color`, which the walk honours.
 *
 ...
 * rule. `nontext.ts` measures those separately, and this page has two that
 * carry meaning: `.disclosure summary::before`, the `▸`/`▾` that is the ONLY
 * affordance on the single `<details>` because the same rule sets
 * `list-style: none` and hides `::-webkit-details-marker`; and the two
 * `.zk-key::before` swatches, which are the legend keys for the masking
 * histogram.
 *
 * SECOND, `aria-hidden` subtrees, which this walk skips to match axe's own
 * boundary. What this lab hides that way is a closed set and was enumerated
 * rather than assumed: the `✓`/`✗` `.check-mark` glyphs (whose meaning is
 * repeated in the `.check-name` beside them, in the same ink on the same
 * surface), the `÷ Z(x) →` and `→` `.q-arrow` connectors, and the numeric
 * `.fri-guide-key` bullets, whose numbers restate the list order. Nothing on
 * this page hides a VALUE — no field element, root, verdict or measured count
 * is inside an `aria-hidden` subtree, which was checked rather than assumed,
 * because that is where both oracles stop looking.
 */
 ...
 * uses. Nothing here is short-lived: `css/style.css` declares no `@keyframes`
 * and no `animation`, and its only hide-transition (`.term-def`, 0.12s) is
 * paired with a `visibility` flip, so every state this gate scans is a settled
 * one.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives 300ms is shorter than a full axe
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * page's `body`: its two radial washes run over a document several screens
     * tall, so text near the top sits on a different surface than text near the
     * bottom. So each gradient is *sampled* at the text's real location
     ...
     * load-bearing here: `body` paints
     * `radial-gradient(1200px 700px at 5% -10%, var(--bg-spot-a) …)` and
     * `radial-gradient(1000px 500px at 95% -5%, var(--bg-spot-b) …)` under every
     * panel, and the `--overlay-*` panel fills above them are translucent
     * (0.94 down to 0.65 alpha), so both show through. It is what makes the
     * SAME `.badge-real` measure 2.90:1 at the top of the page and 3.33:1
     * four exhibits down. The linear branch matters too: `.card` and
     * `.hero-panel` are `linear-gradient(…, var(--overlay-a), var(--overlay-b))`.
     */
     ...
     *
     * A driven pass here walks seven exhibits, four tables, and — the expensive
     * part — an SVG folding diagram of up to 128 `<circle>` dots plus their fold
     * connectors, and a 16-column masking histogram, so hundreds of sibling
     * elements all re-walk the same ancestors up to `<body>`. Without the caches
     ...
     * that is the five `pre.log` regions (`max-height: 220px; overflow: auto`,
     * which the prover transcript and the query inspector both exceed), the
     * four `.table-wrap`s (`overflow-x: auto` around a 660px-min-width table),
     * every `.panel` and `.chart-bar-wrap` (`overflow: hidden`), and
     * `.disclosure`. `.sr-only` is handled separately below.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // exactly the shape that guard exists for — and so is this page's own FRI
      // diagram, whose `<line>` fold connectors are stroke-only and cross every
      // `<text class="viz-label">` row label. Without the FILLED test those
      // lines would read as opaque black rectangles under the labels.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="disclosure">` — this page has exactly one, "What a
      // production STARK still adds", and it ships shut. The gate opens it by
      ...
      // not merely inherited. `css/style.css` defines `.sr-only` as
      // `position:absolute; width:1px; height:1px; clip:rect(0,0,0,0)`, and
      // EIGHT external links carry a `<span class="sr-only"> (opens in new
      // tab)</span>` — four in the hero, four in the cross-demo panel. Every one
      // has a rect and passes `checkVisibility()` while painting exactly zero
     ...
     *
     * What this lab hides is a closed set and every member of it was checked:
     * the `✓`/`✗` `.check-mark` glyphs in the verifier report, whose meaning is
     * repeated verbatim in the `.check-name` beside them in the same ink on the
     * same surface; the `÷ Z(x) →` and `→` `.q-arrow` connectors in the
     * quotient flow; and the numeric `.fri-guide-key` bullets, which restate
     * the list order. All three are decorative repeats of adjacent text.
     *
     * Nothing on this page hides a VALUE. No field element, Merkle root, FRI
     * layer, verdict, transcript line or measured count is inside an
     ...
     *
     * This lab has one real SVG — the FRI folding diagram `renderFriViz()`
     * assembles as an `innerHTML` template literal — and that is exactly how
     * the mistake usually arrives. Interpolating a string ARRAY into a template
     * literal makes JS join it with commas, leaving a run of "," text nodes
     * inside whatever element wraps it; `renderFriViz` builds `lines`, `dots`
     * and `labels` as strings for that reason, and the day one becomes an array
     * those commas would render nothing while a walk that measured them
     * reported a 1:1 failure describing ink that was never laid down.
     */
     ...
     *
     * It is NOT a no-op here. `<body>`'s background is
     * `radial-gradient(…), radial-gradient(…), var(--bg)`, so the canvas colour
     * is a composite this helper has to sample rather than a flat value — and
     * `body { min-height: 100vh }` sits one word away from the `height: 100%`
     * that would leave every element below the first screen intersecting
     * neither box. The day that lands, a walk without this would report the
     * whole page against a white background that does not exist, in a lab whose
     * default theme is near-black.
     */
