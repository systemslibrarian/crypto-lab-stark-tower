 ...
 * hand-measured findings in this sweep, and for seven findings in this repo:
 * every `.bs` button and every `.num-input` select drew its only edge with
 * `--bord`, a SURFACE divider, which against the `--surf` panel they sit on
 * measured 1.22:1 in dark and 1.53:1 in light. This repo had no 1.4.11 check at
 * all before, and no control-boundary token either; `--ctl` was added for it.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * over the farthest-corner radius. Both branches are live here: `<body>`
     * paints two radial washes under every control, and `.card`/`.hero-panel`
     * are `linear-gradient(…, var(--overlay-a), var(--overlay-b))`. Shared
     * verbatim with `contrast.ts` so the two oracles cannot disagree about what
     * colour a surface is.
     */
     ...
     *
     * A driven pass here walks seven exhibits, four tables, an SVG folding
     * diagram of up to 128 dots and a 16-column histogram, so hundreds of
     * sibling elements re-walk the same ancestors up to `<body>`. Without the
     ...
     * what a page paints. A sibling lab in this fleet reports `rgba(0,0,0,0)`
     * for both `html` and `body` because its dark canvas is two radial
     * gradients; the walk found nothing opaque, fell through to WHITE, and
     * reported a bright cyan disclosure triangle at 3.95:1 against a page that
     * is actually near-black. THIS PAGE IS THAT SHAPE: `<body>`'s background is
     * two radial gradients over `var(--bg)`, so the canvas has to be sampled
     * rather than read. The tell for the bug was that the number came out
     * IDENTICAL in both themes — a ratio that cannot depend on the theme is not
     * measuring the theme. The sampler is borrowed from `contrast.ts` wholesale
     * rather than approximated a second time, so the two oracles cannot
     * disagree.
     */
